...
\textbf{Services:} Apply accessibility settings; post announcements via live region or screen-reader API; validate UI elements, producing ValidationReports. \\
...
\textbf{Secrets:} ASR configuration (AsrConfig), model state, buffering strategy, and decoding pipeline producing Transcripts. \\
...
\textbf{Secrets:} Intent schema registry (IntentSchemaRegistry), dialog context management, and intent matching and slot extraction logic. \\
\textbf{Services:} Initialize interpreter; map Transcripts to Intents with slots and confidence scores; reset parsing context. \\
\textbf{Implemented By:} IntentInterpreter module using registered schemas and dialog state. \\
