...
\textbf{Secrets:} How UI state, layout, and visible components are represented, including the mapping from raw browser events into UiEvents and management of keyboard and screen-reader focus. \\
\textbf{Services:} Initialize and render UI; receive and normalize input events into UiEvents; route feedback items; update keyboard/screen-reader focus. \\
...
\textbf{Secrets:} Encoding of accessibility configuration (AccessConfig), ARIA attribute mappings, and validation rules for UI elements against WCAG standards. \\
\textbf{Services:} Apply accessibility options; announce messages; check UI elements and report any problems using ValidationReports. \\
...
\textbf{Secrets:} Internal representation of FeedbackItems keyed by UUID, and policies for creation, clearance, and recovery options of feedback messages. \\
\textbf{Services:} Attach to parent UI; show messages of given types; clear messages; create recovery options for feedback items. \\
...
\textbf{Secrets:} Configuration (AsrConfig) and internal processing details of the speech-to-text engine that converts audio input into accurate text transcript. \\
\textbf{Services:} Initialize engine; process audio to produce transcripts with confidence metadata; reset internal state. \\
\textbf{Implemented By:} SpeechToTextEngine module wrapping acoustic and language models. \\

\medskip
\subsubsection*{M5: Intent Interpreter}
\textbf{Secrets:} How the system recognizes user intentions by using stored intent templates, managing conversation context, and extracting key details (slots) from the user’s speech. \\
\textbf{Services:} Initialize the interpreter; convert speech transcripts into structured intentions, including important details and confidence levels; reset the interpreter’s context when needed. \\
...
\textbf{Secrets:} Command registry structure, user preferences, and policy application for mapping intents to executable commands. \\
\textbf{Services:} Initialize mapping; map Intents to validated Commands; validate commands against guardrails; reset internal state. \\
...
\textbf{Secrets:} Expected error types, message templates and localization hooks, mapping of internal/domain errors to user-facing message keys and recovery options, redaction rules for logs/UI. \\
\textbf{Services:} (1) Display error notification. (2) Display recovery prompt with options. (3) Dismiss item. (4) Log structured error event. (5) Normalize/map errors to UI message + recovery actions. \\
...
\textbf{Secrets:} Device-level audio capture configuration, sampling parameters, buffering strategy, fallback device selection, and how raw audio is normalized before downstream processing.\\
\textbf{Services:} Start/stop microphone stream; read audio frames; provide normalized PCM buffer; expose current device status; surface hardware errors.\\
\textbf{Implemented By:} Browser audio APIs or OS audio interface.\\

\subsubsection*{M19: VAD Noise Filter}
\textbf{Secrets:} Voice-activity-detection thresholds, noise-suppression heuristics, smoothing windows, and adaptive gain rules.\\
\textbf{Services:} Filter raw microphone frames; determine speech vs. silence; emit clean audio frames; send VAD events (start/stop speech).\\
\textbf{Implemented By:} DSP/VAD library.\\


\subsection*{Personalization  Layer}
\subsubsection*{M20: Prompting Module}
\textbf{Secrets:} Prompt templates, personalization rules, condition-based variation (user history, context), and confidence-based prompt selection logic.\\
\textbf{Services:} Generate user prompts; provide clarifying follow-ups when intent confidence is low; produce culturally inclusive and accessibility-enabled copy.\\
\textbf{Implemented By:} Application prompt engine.\\

\subsubsection*{M21: Model Tuner}
\textbf{Secrets:} Adaptation strategy for user-specific speech patterns, tuning weights for model updates, online vs. offline update thresholds, and embedding storage rules.\\
\textbf{Services:} Update inference parameters based on user data; compute personalized embeddings; adjust thresholds for intent classification and STT confidence.\\
...
\textbf{Secrets:} Internal representation of user-custom commands, schema for mapping instructions to intents, and safety constraints preventing unsafe or ambiguous command creation.\\
\textbf{Services:} Store/update/delete user instructions; resolve user-defined commands during intent parsing; validate new instructions; expose registry lookup API.\\
