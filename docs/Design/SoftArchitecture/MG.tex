...
\end{itemize}
\textbf{Implemented By:} \texttt{src/control/browser\_orchestrator.py}
...
\end{itemize}
\textbf{Implemented By:} \texttt{src/control/session\_manager.py}
...
\textbf{Implemented By:} \texttt{audio\_capture.py} (\texttt{AudioConfig}, \texttt{AudioState}, \texttt{RealtimeAudioCapture}) using the \texttt{sounddevice} and \texttt{webrtcvad} libraries.
...
\end{itemize}
\textbf{Implemented By:} \texttt{whisper\_lora\_trainer.py} (\texttt{WhisperLoRATrainer}) using HuggingFace \texttt{transformers}, \texttt{peft} (LoRA), and PyTorch.
...
\end{itemize}
\textbf{Implemented By:} \texttt{ShortcutManager} class in the control/runtime layer with JSON-backed file persistence.
