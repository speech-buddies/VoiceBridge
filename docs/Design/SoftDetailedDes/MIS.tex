...
Apr 01, 2026 & 2.3 & Updated M15 interface specification to match \texttt{audio\_capture.py} \\
Apr 01, 2026 & 2.4 & Updated M18 interface specification to match \texttt{whisper\_lora\_trainer.py} \\
\bottomrule
\end{tabularx}
...
\begin{itemize}
  ...
  \item Browser Orchestrator (M7) / rendering engine to perform DOM updates and capture input events.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
  \item Session Manager (M8) to obtain the active \texttt{Browser} instance used for command execution.
...
\subsubsection{Assumptions}
\begin{itemize}
  \item A valid, started \texttt{Browser} instance is provided as input (typically from M8).
...
\subsection{Uses:}
\begin{itemize}
  \item Speech-to-Text Engine (M4) — consumer of the float32 audio arrays emitted via the \texttt{on\_audio\_ready} callback.
  \item AuditLogger (M11) — stream and VAD error counts are available through \texttt{get\_stats} for diagnostic logging.
\end{itemize}
...
\subsection{Uses:}
\begin{itemize}
  \item Data Management Layer (M9) — source of labelled audio files and the seen-files ledger (\texttt{seen\_files.json}) stored in \texttt{output\_dir}.
  \item ASR Engine (M4) — the LoRA adapter produced by \texttt{save()} is loaded by M4 on the next inference session, adapting recognition to the current user's speech patterns.
  \item AuditLogger (M11) — per-epoch loss values, evaluation results, and save events may be forwarded via the \texttt{on\_epoch\_end} / \texttt{on\_batch\_end} callbacks for audit logging.
\end{itemize}
...
\item AuditLogger (M11) indirectly through logging of shortcut lifecycle events and failures.
\end{itemize}
