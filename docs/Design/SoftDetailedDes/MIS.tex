...
IR & Integrity Requirement \\ \hline
MIS & Module Interface Specification \\ \hline
PCM & Pulse Code Modulation \\ \hline
RMS & Root Mean Square \\ \hline
VAD & Voice Activity Detection \\ \hline
\end{tabular}
...
\textbf{Module:} RealtimeAudioCapture

\subsection{Uses:}
\begin{itemize}
\item Logger (M11) for state transitions, errors, and capture statistics.
\item External libraries: sounddevice for audio streaming, webrtcvad for voice activity detection.
\end{itemize}
...
\texttt{AudioConfig} = \{sample\_rate, channels, chunk\_duration\_ms, vad\_aggressiveness, \\
\hspace*{3cm} pre\_buffer\_duration\_ms, silence\_duration\_ms, max\_recording\_duration\_s, \\
\hspace*{3cm} energy\_threshold, device\}
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{\_\_init\_\_} & config: AudioConfig, & - & ValueError \\ 
 & callback: Callable & & \\
\texttt{start} & - & - & RuntimeError \\ 
\texttt{stop} & - & - & - \\ 
\texttt{get\_stats} & - & Dict[str, Any] & - \\
\texttt{list\_audio\_devices} & - & List[Tuple] & - \\ \hline
\end{tabular}
...
\subsubsection{State Variables:}
\begin{itemize}
\item \texttt{config}: AudioConfig - audio capture and VAD configuration parameters
\item \texttt{state}: AudioState - current capture state
\item \texttt{vad}: webrtcvad.Vad - voice activity detector instance
\item \texttt{pre\_buffer}: deque - circular buffer storing recent audio chunks before speech detection
\item \texttt{recording\_buffer}: list - accumulates audio chunks during active recording
\item \texttt{silence\_counter}: int - tracks consecutive silent chunks
\item \texttt{recording\_chunks\_count}: int - counts chunks in current recording
\item \texttt{stream}: sounddevice.InputStream - active audio input stream
\item \texttt{stats}: dict - capture statistics (total recordings, audio seconds, errors)
\end{itemize}
...
\subsubsection{Assumptions:}
\begin{itemize}
\item Audio device supports requested sample rate (8000, 16000, 32000, or 48000 Hz)
\item Chunk duration is 10, 20, or 30ms (required by WebRTC VAD)
\item Callback function \texttt{on\_audio\_ready} processes audio asynchronously
\end{itemize}

\subsubsection{Access Routine Semantics:}

\noindent\texttt{\_\_init\_\_(config, on\_audio\_ready)}:
\begin{itemize}
\item \textbf{transition:} Initializes VAD with specified aggressiveness level (0-3), validates configuration parameters, sets state to IDLE.
\item \textbf{output:} None
\item \textbf{exception:} ValueError if configuration validation fails (invalid sample rate or chunk duration).
\end{itemize}

\noindent\texttt{start()}:
\begin{itemize}
\item \textbf{transition:} Creates and starts sounddevice InputStream, transitions state from IDLE to LISTENING. Begins continuous audio capture with callback processing.
\item \textbf{output:} None
\item \textbf{exception:} RuntimeError if audio stream fails to start or if already running.
\end{itemize}

\noindent\texttt{stop()}:
\begin{itemize}
\item \textbf{transition:} Transitions state to STOPPED, stops and closes audio stream, clears buffers, returns state to IDLE.
\item \textbf{output:} None
\end{itemize}

\noindent\texttt{get\_stats()}:
\begin{itemize}
\item \textbf{output:} Dictionary containing total\_recordings, total\_audio\_seconds, vad\_errors, stream\_errors.
\end{itemize}

\noindent\texttt{list\_audio\_devices()} (static):
\begin{itemize}
\item \textbf{output:} List of tuples (device\_index, device\_info) for all available input devices.
\end{itemize}

\subsubsection{Internal Processing (State Machine):}

The module implements a state machine that processes audio chunks through the following logic:

\begin{itemize}
\item \textbf{LISTENING}: Maintains pre-buffer of recent chunks. On speech detection → SPEECH\_DETECTED.
\item \textbf{SPEECH\_DETECTED}: Copies pre-buffer to recording buffer → RECORDING.
\item \textbf{RECORDING}: Accumulates audio chunks. Increments silence\_counter when no speech detected. Ends recording when:
  \begin{itemize}
  \item Silence duration exceeds threshold (default 1500ms), or
  \item Maximum recording duration reached (default 30s)
  \end{itemize}
\item \textbf{PROCESSING}: Converts accumulated int16 audio to float32, invokes callback with audio array and metadata, then → LISTENING.
\end{itemize}
...
\noindent\texttt{\_is\_speech(audio\_chunk: bytes) → bool}:
\begin{itemize}
\item Applies WebRTC VAD to determine if chunk contains speech.
\item If energy\_threshold configured, also computes RMS energy and returns OR of both criteria.
\item Increments vad\_errors on exception.
\end{itemize}

\noindent\texttt{\_process\_audio\_chunk(audio\_chunk: bytes)}:
\begin{itemize}
\item Implements state machine logic based on current state and speech detection result.
\end{itemize}

\noindent\texttt{\_start\_recording()}:
\begin{itemize}
\item Initializes recording\_buffer with pre\_buffer contents.
\end{itemize}

\noindent\texttt{\_end\_recording()}:
\begin{itemize}
\item Concatenates recording buffer, converts to float32, creates metadata, invokes callback.
\item Updates statistics and resets recording state.
\end{itemize}

\noindent\texttt{\_audio\_callback(indata, frames, time\_info, status)}:
\begin{itemize}
\item Sounddevice callback invoked for each audio chunk.
\item Converts float32 input to int16 bytes and forwards to \_process\_audio\_chunk.
\end{itemize}
