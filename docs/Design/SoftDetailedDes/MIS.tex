...
\texttt{applySettings} & \texttt{settings: AccessConfig} & - & - \\ 
\texttt{announce} & \texttt{msg: string} & - & - \\ 
...
\begin{itemize}
  ...
  \textbf{Description:} \texttt{check\_contrast(fg, bg)} returns \texttt{true} if contrast ratio between foreground \texttt{fg} and background \texttt{bg} meets WCAG level.
  ...
  \textbf{Description:} \texttt{aria\_set(elemId, metadata)} returns \texttt{true} if ARIA attributes from \texttt{metadata} are applied to element \texttt{elemId}.
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{BrowserController} & \texttt{bridge: Client} & self & - \\
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{createProfile} & userToken, initData & profileID & ProfileCreationException \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{authenticateUser} & username, password & sessionToken & AuthException \\ 
\texttt{validateToken} & sessionToken & validity & TokenException \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{encryptData} & plainData, keyID & cipherData & EncryptionException \\ 
\texttt{decryptData} & cipherData, keyID & plainData & DecryptionException \\ 
\texttt{rotateKeys} & rotationPolicy & result & RotationFailureException \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{validateCommand} & commandText, context & validationResult & InvalidCommandException \\ 
\texttt{reportIncident} & incidentData & reportID & ReportFailureException \\ 
...
\texttt{startCapture} & deviceID & status & PermissionException \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{filterFrame} & audioFrame & cleanedFrame & - \\ 
\texttt{detectSpeech} & audioFrame & isSpeech (bool) & - \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{makePrompt} & uiState & promptText & - \\ 
\texttt{makeConfirm} & intent & promptText & - \\ 
...
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{lookup} & intent & instruction & NotFoundException \\ 
\texttt{registerInstr} & instruction & status & StoreException \\ 
