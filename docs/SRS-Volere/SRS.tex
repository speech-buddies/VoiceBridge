...
\title{
    ...
    Software Requirements Specification for VoiceBridge: An Accessible Speech-to-Control System
} 
\author{\authname}
\date{\today}
...
This section summarizes the major revisions made to the SRS over time, including document completion, review cycles, and later feedback-driven updates.
...
December 24, 2025 & Luna & Addressing TA feedback: adding requirement names, hyperlinks and minor fixes\\
December 25, 2025 & Luna & Addressing Peer feedback: expanding rationale behind choice of 16Hz in FR1\\
January 10, 2026 & Mazen & Linked SRS to Hazard Analysis Requirements for consistency\\
April 4, 2026 & Luna & Addressed final TA comments\\
...
This section identifies the groups and individuals who influence, use, fund, support, or are affected by VoiceBridge. It clarifies their roles and priorities to ensure project decisions remain aligned with user needs, accessibility goals, and long-term maintainability.
...
\begin{itemize}
  \item \textbf{Amira}, a 45-year-old with Parkinson’s disease, experiences fluctuating speech clarity and occasional motor fatigue that make typing difficult during long sessions. She uses VoiceBridge to log into Gmail, draft and send emails, and navigate between inbox folders with minimal manual input. Her priorities are accurate transcription, reliable command confirmation, and clear feedback that helps her recover quickly from recognition errors.
  \item \textbf{David}, a stroke survivor, has mild dysarthria and reduced fine-motor control in one hand, which slows traditional mouse-and-keyboard interaction. He uses VoiceBridge to browse websites, search for products, compare options, and complete online purchases with greater independence. His priorities are simple command phrasing, low-latency responses, and safe guardrails for high-impact actions such as checkout or form submission.
\end{itemize}
...
This section defines the non-negotiable constraints that bound VoiceBridge design and implementation, including technical environment, integration dependencies, schedule and budget limits, and compliance expectations. These constraints establish the operational limits within which all requirements must be satisfied.
...
This subsection defines constraints related to the baseline operating environment and platform support expected for VoiceBridge deployment.

\noindent\textbf{C3 — The product shall run on consumer-grade hardware using a personal computer microphone and support macOS, Windows, and Linux distributions.}\\
...
This section defines the key terms, acronyms, and naming conventions used throughout the VoiceBridge SRS. It provides a shared vocabulary so stakeholders can interpret requirements, constraints, and design discussions consistently.
...
This section captures the key facts and assumptions that shape VoiceBridge requirements and design choices. It distinguishes validated context from expected operating conditions so requirements can be interpreted consistently throughout implementation and testing.

\subsection{Relevant Facts}
The facts in this subsection summarize the core user, environment, and usage realities that directly influence VoiceBridge requirements. Table \ref{tab:relevant-facts} consolidates these validated considerations so later functional, usability, and performance requirements remain grounded in the project context.
...
The business rules in this subsection define the operational policies that govern how VoiceBridge should behave during user interaction. Table \ref{tab:business-rules} summarizes the key rules that shape confirmation flows, personalization behavior, reliability expectations, and protection against unintended actions.
...
BR6 & Users should be able to configure simple command mappings for personalized tasks. & Supports individual preferences and improves user autonomy. These personalized mappings will be stored locally on the user's device in user-specific files rather than in a remote database. (\hyperref[tab:project-goals]{G2}, \hyperref[tab:project-goals]{G3}, \hyperref[tab:project-goals]{G8}) \\
\bottomrule
...
Table \ref{tab:assumptions} summarizes the main assumptions that shape the expected operating conditions for VoiceBridge and their implications for design decisions.
...
This section outlines the current problem context, the operational setting in which VoiceBridge is introduced, and the major work activities the system must support. It clarifies how the project fits into existing user workflows and defines the boundaries of the work being addressed.
...
Figure \ref{fig:voicebridge-context} shows the external context in which VoiceBridge operates, including the primary user, the system boundary, and the surrounding platforms and services it interacts with. This diagram helps clarify the main information and control flow between the user, VoiceBridge, and adjacent systems within the project scope.
...
Table \ref{tab:work-partitioning} summarizes the major work events in VoiceBridge, along with their inputs, outputs, corresponding business use case summaries, and relevant data classes. It provides a high-level breakdown of how the system workflow is partitioned across the main stages of speech capture, interpretation, confirmation, and browser interaction.
...
\label{tab:work-partitioning}
\end{tabularx}
\end{table}

\subsection{Specifying a Business Use Case (BUC)} 
Table \ref{tab:buc-summary} lists the primary business use cases that structure the VoiceBridge workflow, from capturing user speech to executing the resulting browser command. It provides a concise summary of the main user-system interactions that are elaborated further in the detailed BUC figures and descriptions that follow.
...
This section defines the core data objects, transformations, and relationships that support VoiceBridge functionality. It first presents the high-level business data model for how speech input moves through the system, then documents the main data elements used for transcription, intent interpretation, browser command execution, feedback, and optional local personalization.

\subsection{Business Data Model}
The business data model illustrated in Figure \ref{fig:voicebridge-usecase} defines the core VoiceBridge pipeline from raw audio input to browser command execution. To support a lightweight and accessible design (\hyperref[tab:project-goals]{G4}), the system processes atypical speech through the speech-to-text pipeline to produce a transcript, then passes that transcript to the intent and command-mapping components that generate a standardized browser action. This architecture keeps the main interaction flow simple and privacy-conscious by treating speech input as transient runtime data, while allowing optional personalization data to remain stored locally on the user's device in accordance with the project's privacy goals (\hyperref[tab:project-goals]{G7}).
...
Table \ref{tab:data-dictionary} defines the primary data elements used throughout the VoiceBridge pipeline, including speech input, intermediate recognition outputs, interpreted intents, executable commands, feedback messages, and locally stored personalization data. It serves as a reference for the meaning, source, type, and system usage of each core data object.
...
\hline
\textbf{User\_Profile} & Speaker-specific data such as ID, preferences, speech patterns, and personalized command mappings & JSON object & Local user-specific JSON files on the host device & Enables personalized recognition and response without requiring a remote database \\
\hline
...
This section defines the functional boundary of VoiceBridge as a product, identifying the major system capabilities it includes and the external interactions it depends on. It introduces the product boundary representation and the key product use cases that describe how users interact with the system to capture speech, confirm intent, execute commands, and receive feedback.
...
Table \ref{tab:product-use-cases} summarizes the main product use cases supported by VoiceBridge and the key inputs, outputs, triggers, and outcomes associated with each one.
...
This section identifies common failure modes and abnormal operating conditions that may arise during VoiceBridge use. It outlines likely causes, expected system behavior, severity, and the feedback provided to users when these events occur.

Table \ref{tab:undesired-events} summarizes the primary undesired events considered for VoiceBridge, along with their causes, handling strategies, severity levels, and user-facing notifications.
...
This section specifies the core system behaviors VoiceBridge must provide to satisfy its primary purpose as an accessible speech-to-control product. The following functional requirements define how the system captures speech, converts it to text, presents it for verification, maps confirmed input to executable commands, performs those commands, and responds to execution constraints or failures.
...
\bigskip
\subsection{FR2: Convert Impaired Speech to Text with greater than or equal to 80\% Accuracy (MVP)}\label{fr:FR2}
...
\bigskip
\subsection{FR3: Display Transcription for Verification}\label{fr:FR3}
...
\bigskip
\subsection{FR4: Map Text to Arbitrary Device Commands}\label{fr:FR4}
...
\bigskip
\subsection{FR5: Execute Commands on the Host Device}\label{fr:FR5}
...
\bigskip
\subsection{FR6: Handle Security Software Interference}\label{fr:FR6}
...
\section{Look and Feel Requirements}\label{sec:look-feel}
This section describes the visual and stylistic expectations for the VoiceBridge interface. It focuses on clarity, minimal distraction, and presentation choices that support accessible and effective user interaction.

\subsection{Appearance Requirements}\label{subsec:appearance}

\textbf{AR1: Minimal and Non-Intrusive Interface}\label{req:AR1}
...
\subsection{Style Requirements}\label{subsec:style}

\textbf{SR1: Professional Visual Style}\label{req:SR1}
...
\section{Usability and Humanity Requirements}\label{sec:usability-humanity}
This section defines the human-centered qualities VoiceBridge should exhibit in everyday use, including ease of use, learnability, personalization potential, clarity of communication, and accessibility support.

\subsection{Ease of Use Requirements}\label{subsec:ease-of-use}
\textbf{EU1: Low Interaction Effort}\label{req:EU1}
...
\subsection{Personalization and Internationalization Requirements}\label{subsec:personalization}

\textbf{PI1: Personalization and Language Support}\label{req:PI1}
...
\subsection{Learning Requirements}\label{subsec:learning}

\textbf{LR1: Rapid Learnability}\label{req:LR1}
...
\subsection{Understandability and Politeness Requirements}\label{subsec:understandability}

\textbf{UP1: Clear System Communication}\label{req:UP1}
...
\subsection{Accessibility Requirements}\label{subsec:accessibility}

\textbf{AC1: Assistive Tool Compatibility}\label{req:AC1}
...
This section defines the performance expectations for VoiceBridge, including response time, accuracy, reliability under failure conditions, capacity, scalability, and long-term operability.

\subsection{Speed and Latency Requirements}\label{subsec:speed-latency}

\textbf{PR1: Bounded Speech-to-Action Latency}\label{req:PR1}
...
\subsection{Safety-Critical Requirements}\label{subsec:safety}

\textbf{PR2: Prevention of Unsafe Actions}\label{req:PR2}
...
\subsection{Precision or Accuracy Requirements}\label{subsec:accuracy}

\textbf{PR3: Minimum Recognition Accuracy}\label{req:PR3}
...
\subsection{Robustness or Fault-Tolerance Requirements}\label{subsec:robustness}

\textbf{PR4: Degradation Under Failure}\label{req:PR4}
...
\subsection{Capacity Requirements}\label{subsec:capacity}

\textbf{PR5: Concurrent User Support}\label{req:PR5}

The system shall support at least 20 concurrent users without service degradation if the product is commercialized.
\subsection{Scalability or Extensibility Requirements}\label{subsec:scalability}

\textbf{PR6: Scalable Architecture}\label{req:PR6}
...
\subsection{Longevity Requirements}\label{subsec:longevity}

\textbf{PR7: Long-Term Operability}\label{req:PR7}
...
This section describes the environments in which VoiceBridge is expected to operate and the external systems it must work with. It covers physical conditions, deployment assumptions, integration boundaries, and release considerations.

\subsection{Expected Physical Environment}\label{subsec:physical-env}

\textbf{OE1: Operation in Typical Work Environments}\label{req:OE1}
...
\subsection{Wider Environment Requirements}\label{subsec:wider-env}

\textbf{OE2: Web-Based Deployment Environment}\label{req:OE2}
...
\subsection{Requirements for Interfacing with Adjacent Systems}\label{subsec:adjacent-systems}

\textbf{OE3: Integration with External Systems}\label{req:OE3}
\begin{enumerate}
...
\subsection{Productization Requirements}\label{subsec:productization}

\textbf{OE4: Multi-User Deployment and Distribution}\label{req:OE4}
\begin{enumerate}
...
\end{enumerate}
\subsection{Release Requirements}\label{subsec:release}

\textbf{OE5: Defined Release Cycle}\label{req:OE5}
...
\section{Maintainability and Support Requirements}\label{sec:maintainability}
This section defines the qualities needed to keep VoiceBridge maintainable, supportable, and adaptable over time. It addresses expectations for code quality, user support resources, and platform compatibility.

\subsection{Maintenance Requirements}\label{subsec:maintenance}

\textbf{MS1: Maintainable Codebase}\label{req:MS1}
...
\subsection{Supportability Requirements}\label{subsec:supportability}

\textbf{MS2: User Support Resources}\label{req:MS2}
...
\subsection{Adaptability Requirements}\label{subsec:adaptability}
\textbf{MS3: Platform Compatibility}\label{req:MS3}
...
This section defines cultural and ethical expectations for how VoiceBridge should communicate with users. It emphasizes respectful, inclusive, and non-discriminatory interaction across diverse user populations.

\subsection{Cultural Requirements}\label{subsec:cultural}

\textbf{CR1: Culturally Neutral Interaction}\label{req:CR1}
...
This section identifies the legal and standards-based obligations that apply to VoiceBridge. It ensures that privacy handling and accessibility expectations remain aligned with recognized external requirements.

\subsection{Legal Requirements}\label{subsec:legal}

\textbf{CO1: Legal Compliance}\label{req:CO1}
...
\subsection{Standards Compliance Requirements}\label{subsec:standards}

\textbf{CO2: Accessibility Standards Compliance}\label{req:CO2}
...
This section distinguishes between requirements that are expected to evolve as the project matures and those that are considered stable foundations of the product. It helps communicate where future refinement is most likely.
...
This section summarizes existing products and reusable components that may inform or support VoiceBridge development. It highlights candidate technologies that can serve as baselines, integrations, or implementation shortcuts.
...
This section outlines new issues or limitations that VoiceBridge itself may introduce into the user or deployment environment. It highlights risks, tradeoffs, and follow-up concerns that should be considered during implementation and adoption.
...
\section{Tasks}
This section summarizes the major categories of work required to deliver VoiceBridge. It provides a high-level view of planning, implementation, and development-phase organization.
...
This section describes how users and their data will transition to VoiceBridge from initial onboarding through broader adoption. It addresses migration support, account continuity, and access to previously associated data.

\subsection{User Onboarding and Migration}
...
\subsection{Data Support Across Accounts}
...
\section{Costs}
The primary costs for this product are related to machine learning computation and hosting. The project budget is set by the department, and eligible expenses are reimbursed by the department according to its reimbursement process. Development and training will initially leverage Compute Canada credits provided by the supervising research team.
...
This section defines the documentation and onboarding support that should accompany VoiceBridge. It clarifies expectations for self-directed learning materials and user-facing guidance.
...
This section captures candidate solution directions and future design possibilities that may support VoiceBridge development. These ideas are exploratory and complement, rather than replace, the formal requirements defined earlier in the document.
...
\begin{enumerate}
    ...
    \textit{Phase: PoC} \\
    \textit{Rationale:} Core functionality must be demonstrated to validate feasibility. See \hyperref[sec:FR]{Functional Requirements}.

    % Look and Feel
    \item \textbf{10.1 Appearance Requirements:} Initial UI layout and interface elements.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Visual interface can be refined after core functions are stable. See \hyperref[subsec:appearance]{Appearance Requirements}.

    \item \textbf{10.2 Style Requirements:} Consistent fonts, colors, and spacing.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Enhances readability and user experience once functionality is proven.See \hyperref[subsec:style]{Style Requirements}.

    % Usability
    \item \textbf{11.1 Ease of Use Requirements:} Interface must be intuitive and straightforward.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Important for usability testing, applied after functional PoC is complete. See \hyperref[subsec:ease-of-use]{Ease of Use Requirements}.

    \item \textbf{11.2 Personalization and Internationalization Requirements:} Language/voice adaptations.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Applied later when system is functionally stable. See \hyperref[subsec:personalization]{Personalization Requirements}.

    \item \textbf{11.3 Learning Requirements:} System should adapt to repeated user patterns.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Machine learning or adaptation features require validated data pipelines. See \hyperref[subsec:learning]{Learning Requirements}.

    \item \textbf{11.4 Understandability and Politeness Requirements:} Feedback should be clear and respectful.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Improves user experience during early testing. See \hyperref[subsec:understandability]{Understandability and Politeness Requirements}.


    \item \textbf{11.5 Accessibility Requirements:} Interface supports assistive technologies.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Early implementation ensures PoC testing is inclusive. See \hyperref[subsec:accessibility]{Accessibility Requirements}.

    % Performance
    \item \textbf{12.1 Speed and Latency Requirements:} System responds within acceptable time frames.\\
    \textit{Phase: PoC} \\
    \textit{Rationale:} Performance validation is crucial for feasibility. See \hyperref[subsec:speed-latency]{Speed and Latency Requirements}.

    \item \textbf{12.2 Safety-Critical Requirements:} Ensure no harmful outputs or behaviors.\\
    \textit{Phase: PoC} \\
    \textit{Rationale:} Early validation prevents critical errors in PoC. See \hyperref[subsec:safety]{Safety-Critical Requirements}.

    \item \textbf{12.3 Precision or Accuracy Requirements:} Transcription must meet minimum WER/CER.\\
    \textit{Phase: PoC} \\
    \textit{Rationale:} Accuracy is the central PoC metric. See \hyperref[subsec:accuracy]{Precision or Accuracy Requirements}.

    \item \textbf{12.4 Robustness or Fault-Tolerance Requirements:} Handle errors gracefully.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Necessary for reliable system use beyond PoC. See \hyperref[subsec:robustness]{Robustness or Fault-Tolerance Requirements}.

    \item \textbf{12.5 Capacity Requirements:} Support a small set of concurrent users/data.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Ensure system can handle expected early usage. See \hyperref[subsec:capacity]{Capacity Requirements}.

    \item \textbf{12.6 Scalability or Extensibility Requirements:} System should allow future growth.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Added after functional validation for longer-term planning. See \hyperref[subsec:scalability]{Scalability or Extensibility Requirements}.

    \item \textbf{12.7 Longevity Requirements:} Components maintainable over time.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Stability and maintainability are late-stage concerns. See \hyperref[subsec:longevity]{Longevity Requirements}.

    % Operational & Environment
    \item \textbf{13.1 Expected Physical Environment:} System works on typical office/lab computers.\\
    \textit{Phase: PoC} \\
    \textit{Rationale:} Early testing on target devices is essential. See \hyperref[subsec:physical-env]{Expected Physical Environment}.

    \item \textbf{13.2 Wider Environment Requirements:} Browser compatibility.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Once core functionality works, broaden environment testing. See \hyperref[subsec:wider-env]{Wider Environment Requirements}.

    \item \textbf{13.3 Requirements for Interfacing with Adjacent Systems:} Integration points identified.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} External integration is late-phase after internal systems are stable. See \hyperref[subsec:adjacent-systems]{Requirements for Interfacing with Adjacent Systems}.

    \item \textbf{13.4 Productization Requirements:} Ready for deployment.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Combines all prior validations for release. See \hyperref[subsec:productization]{Productization Requirements}.

    \item \textbf{13.5 Release Requirements:} Packaging, distribution readiness.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Ensures final delivery meets standards. See \hyperref[subsec:release]{Release Requirements}.

    % Maintainability & Support
    \item \textbf{14.1 Maintenance Requirements:} System maintainable.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Implemented once core system is functional. See \hyperref[subsec:maintenance]{Maintenance Requirements}.

    \item \textbf{14.2 Supportability Requirements:} Technical support procedures.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Late-stage requirement after main functionality. See \hyperref[subsec:supportability]{Supportability Requirements}.

    \item \textbf{14.3 Adaptability Requirements:} System can be upgraded.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Flexibility comes after base system is validated. See \hyperref[subsec:adaptability]{Adaptability Requirements}.

    % Security
    \item \textbf{15.1 Access Requirements:} User roles and permissions.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Early implementation secures initial PoC data. See \hyperref[subsec:access]{Access Requirements}.

    \item \textbf{15.2 Integrity Requirements:} Prevent tampering or corruption.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Ensures reliable data handling. See \hyperref[subsec:integrity]{Integrity Requirements}.

    \item \textbf{15.3 Privacy Requirements:} Encrypt data in transit and at rest.\\
    \textit{Phase: Revision 0} \\
    \textit{Rationale:} Essential for user trust and legal compliance. See \hyperref[subsec:privacy]{Privacy Requirements}.

    \item \textbf{15.4 Audit Requirements:} Track critical system events.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Auditing implemented after basic functionality and privacy are validated. See \hyperref[subsec:audit]{Audit Requirements}.

    \item \textbf{15.5 Immunity Requirements:} Protection from external attacks.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Security hardening is a late-stage activity. See \hyperref[subsec:immunity]{Immunity Requirements}.

    % Cultural & Compliance
    \item \textbf{16.1 Cultural Requirements:} Respect cultural norms and sensitivities.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Applied after all functional, UI, and content elements are stable. See \hyperref[subsec:cultural]{Cultural Requirements}.

    \item \textbf{17.1 Legal Requirements:} Comply with laws and regulations.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Legal compliance verified after security, privacy, and functionality are validated. See \hyperref[subsec:legal]{Legal Requirements}.

    \item \textbf{17.2 Standards Compliance Requirements:} Follow industry standards.\\
    \textit{Phase: Revision 1} \\
    \textit{Rationale:} Ensures credibility and certification readiness at final stage. See \hyperref[subsec:standards]{Standards Compliance Requirements}.
\end{enumerate}
...
\begin{itemize}
    \item \hyperref[fr:FR1]{FR1}: Accept Speech Input ($Idle \rightarrow Listening$)
    \item \hyperref[fr:FR2]{FR2}: Convert Speech to Text ($Listening \rightarrow Processing$)
    \item \hyperref[fr:FR3]{FR3}: Confirm Transcription ($Processing \rightarrow Confirming$)
    \item \hyperref[fr:FR4]{FR4}: Map Intent to Command ($Confirming \rightarrow Executing$)
    \item \hyperref[fr:FR5]{FR5}: Execute Command ($Executing \rightarrow Idle$)
\end{itemize}
...
This appendix contains the project reflection material associated with the VoiceBridge SRS, including retrospective commentary and supporting reflections referenced by the course deliverables.
