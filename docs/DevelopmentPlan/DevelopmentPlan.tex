...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Produces transcribed text output (does not need to be real time).
    \end{itemize}
...
Week & Activities \\
\midrule
1 & \textbf{Systematic Review:} Perform a systematic review of current state-of-the-art models and methods relating to ASR for atypical speech. \\
\hline
...
\hline
3 & \textbf{Baseline System:} Run selected pre-trained ASR (e.g., Whisper small, wav2vec2.0, or other popular models) directly on impaired speech samples. Measure baseline error rates. \\
\hline
4 & \textbf{Adaptation Phase 1:} Fine-tune a small model (ideally the model selected in previous step) on subset of impaired speech data using transfer learning. Track loss and improvements. \\
\hline
...
\hline
6 & \textbf{Comparison \& Refinement:} Compare baseline vs. fine-tuned performance. Select best performing model/hybrid architectures. Integrate simple demo (held out test data $\rightarrow$ ASR $\rightarrow$ text box). \\
\hline
...
\begin{itemize}
    \item \textbf{Primary:} One or multiple impaired speech datasets:
    \begin{itemize}
        \item TORGO (Parkinson's, ALS patients)
        \item SAP (large dataset of dysarthric speakers)
